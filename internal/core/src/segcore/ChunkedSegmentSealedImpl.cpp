...
struct ManifestColumnGroupBuildTiming {
    int64_t reader_ns = 0;
...
CreateManifestColumnGroup(const ManifestColumnGroupBuildContext& context,
                          ManifestColumnGroupBuildTiming* timing = nullptr) {
    auto stage_start = std::chrono::steady_clock::now();
    auto chunk_reader = context.chunk_reader_factory();
    auto reader_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
    ...
    auto translator_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                             std::chrono::steady_clock::now() - stage_start)
                             .count();

    stage_start = std::chrono::steady_clock::now();
    auto group = std::make_shared<ChunkedColumnGroup>(std::move(translator));
    auto cache_slot_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                             std::chrono::steady_clock::now() - stage_start)
                             .count();

    if (timing != nullptr) {
        timing->reader_ns = reader_ns;
        timing->translator_ns = translator_ns;
        timing->cache_slot_ns = cache_slot_ns;
    }
    return group;
}
...
ApproxLazyManifestColumnGroupBytes(
    const ManifestColumnGroupBuildContext& context) {
...
class LazyManifestColumnGroup {
 public:
    ...
    Materialize() {
        ...
        group_ = CreateManifestColumnGroup(context_);
    ...
    MaterializeColumn(FieldId field_id) {
        auto it = context_.field_metas.find(field_id);
        AssertInfo(it != context_.field_metas.end(),
                   "field {} not found in lazy manifest column group",
                   field_id.get());
        return std::make_shared<ProxyChunkColumn>(
            Materialize(), field_id, it->second);
    }
...
class LazyManifestProxyColumn : public ChunkedColumnInterface {
 public:
    ...
    ManualEvictCache() const override {
        if (group_->NumFieldsInGroup() == 1 && group_->IsMaterialized()) {
            group_->ManualEvictCache();
        }
    }
 ...
 private:
    ...
    MaterializedColumn() const {
        std::lock_guard<std::mutex> lock(mutex_);
        if (column_ == nullptr) {
            column_ = group_->MaterializeColumn(field_id_);
        }
        return column_;
    }
...
struct ApplyLoadDiffTimingStats {
    ...
    std::atomic<int64_t> index_load_ns{0};
    std::atomic<int64_t> index_replace_ns{0};
    std::atomic<int64_t> reload_field_ns{0};
    std::atomic<int64_t> column_group_ns{0};
    std::atomic<int64_t> init_text_lob_ns{0};
    std::atomic<int64_t> binlog_load_ns{0};
    std::atomic<int64_t> binlog_replace_ns{0};
    std::atomic<int64_t> drop_index_ns{0};
    std::atomic<int64_t> text_index_ns{0};
    std::atomic<int64_t> json_stats_ns{0};
    std::atomic<int64_t> default_fill_ns{0};
    std::atomic<int64_t> create_text_index_ns{0};
    std::atomic<int64_t> drop_field_data_ns{0};
    std::atomic<int64_t> index_count{0};
    std::atomic<int64_t> column_group_count{0};
    std::atomic<int64_t> binlog_count{0};
    std::atomic<int64_t> text_index_count{0};
    std::atomic<int64_t> json_stats_count{0};
...
struct SegmentLoadTimingStats {
    ...
    std::atomic<int64_t> start_log_ns{0};
    std::atomic<int64_t> prepare_info_ns{0};
    std::atomic<int64_t> diff_ns{0};
    std::atomic<int64_t> diff_log_ns{0};
    std::atomic<int64_t> apply_ns{0};
    std::atomic<int64_t> success_log_ns{0};
    std::atomic<int64_t> max_total_ns{0};
    std::atomic<int64_t> last_log_ns{0};
};
...
RecordApplyLoadDiffTiming(int64_t total_ns,
                          ...
                          int64_t index_replace_ns,
                          int64_t reload_field_ns,
                          int64_t column_group_ns,
                          int64_t init_text_lob_ns,
                          int64_t binlog_load_ns,
                          int64_t binlog_replace_ns,
                          int64_t drop_index_ns,
                          int64_t text_index_ns,
                          int64_t json_stats_ns,
                          int64_t default_fill_ns,
                          int64_t create_text_index_ns,
                          int64_t drop_field_data_ns,
                          int64_t index_count,
                          int64_t column_group_count,
    ...
    stats.default_fill_ns.fetch_add(default_fill_ns, std::memory_order_relaxed);
    stats.create_text_index_ns.fetch_add(create_text_index_ns,
                                         std::memory_order_relaxed);
    stats.drop_field_data_ns.fetch_add(drop_field_data_ns,
                                       std::memory_order_relaxed);
    stats.index_count.fetch_add(index_count, std::memory_order_relaxed);
    ...
    auto reload_field =
        stats.reload_field_ns.exchange(0, std::memory_order_relaxed);
    auto column_group =
    ...
    auto create_text_index =
        stats.create_text_index_ns.exchange(0, std::memory_order_relaxed);
    auto drop_field_data =
        stats.drop_field_data_ns.exchange(0, std::memory_order_relaxed);
    auto indexes = stats.index_count.exchange(0, std::memory_order_relaxed);
    auto column_groups =
        stats.column_group_count.exchange(0, std::memory_order_relaxed);
    auto binlogs = stats.binlog_count.exchange(0, std::memory_order_relaxed);
    auto text_indexes =
        stats.text_index_count.exchange(0, std::memory_order_relaxed);
    auto json_stats_entries =
        stats.json_stats_count.exchange(0, std::memory_order_relaxed);
    ...
    LOG_WARN(
        ...
        "avgBinlogReplaceMs={:.3f} avgDropIndexMs={:.3f} "
        "avgTextIndexMs={:.3f} avgJsonStatsMs={:.3f} "
        "avgDefaultFillMs={:.3f} avgCreateTextIndexMs={:.3f} "
        "avgDropFieldDataMs={:.3f} avgIndexCount={:.3f} "
        ...
        AvgMs(reload_field, count),
        AvgMs(column_group, count),
        AvgMs(init_text_lob, count),
        AvgMs(binlog_load, count),
        AvgMs(binlog_replace, count),
        AvgMs(drop_index, count),
        AvgMs(text_index, count),
        AvgMs(json_stats, count),
        AvgMs(default_fill, count),
        AvgMs(create_text_index, count),
        AvgMs(drop_field_data, count),
        static_cast<double>(indexes) / static_cast<double>(count),
        static_cast<double>(column_groups) / static_cast<double>(count),
...
RecordSegmentLoadTiming(int64_t total_ns,
    ...
    stats.success_log_ns.fetch_add(success_log_ns, std::memory_order_relaxed);
    UpdateMax(stats.max_total_ns, total_ns);
    ...
    auto total = stats.total_ns.exchange(0, std::memory_order_relaxed);
    auto lock_wait = stats.lock_wait_ns.exchange(0, std::memory_order_relaxed);
    auto snapshot = stats.snapshot_ns.exchange(0, std::memory_order_relaxed);
    auto start_log = stats.start_log_ns.exchange(0, std::memory_order_relaxed);
    auto prepare_info =
        stats.prepare_info_ns.exchange(0, std::memory_order_relaxed);
    auto diff = stats.diff_ns.exchange(0, std::memory_order_relaxed);
    auto diff_log = stats.diff_log_ns.exchange(0, std::memory_order_relaxed);
    auto apply = stats.apply_ns.exchange(0, std::memory_order_relaxed);
    auto success_log =
        stats.success_log_ns.exchange(0, std::memory_order_relaxed);
    auto max_total = stats.max_total_ns.exchange(0, std::memory_order_relaxed);
    ...
    LOG_WARN(
        ...
        AvgMs(start_log, count),
        AvgMs(prepare_info, count),
        AvgMs(diff, count),
        AvgMs(diff_log, count),
        AvgMs(apply, count),
...
ChunkedSegmentSealedImpl::LoadVecIndex(LoadIndexInfo& info, bool is_replace) {
    ...
    if (info.has_load_resource_request) {
...
ChunkedSegmentSealedImpl::LoadColumnGroups(const std::string& manifest_path,
    ...
    // External tables keep one projected reader per warmup-disabled field so
    // accessing one field does not pull sibling columns from object storage.
    ...
    for (const auto& pair : cg_field_ids) {
        ...
        std::vector<FieldId> disabled_fields;
        ...
        for (const auto& field_id : all_fields) {
            ...
            if (resolved != CacheWarmupPolicy::CacheWarmupPolicy_Disable) {
                ...
                disabled_fields.push_back(field_id);
            }
        }

        auto eager_field_count = eager_fields.size();
        auto disabled_field_count = disabled_fields.size();
        if (!eager_fields.empty()) {
            tasks.push_back({cg_index, std::move(eager_fields), true});
        }
        for (const auto& fid : disabled_fields) {
            tasks.push_back({cg_index, {fid}, false});
        }
        if (eager_field_count > 0 && disabled_field_count > 0) {
            LOG_INFO(
                "[LoadColumnGroups] segment {} cg {} split: {} eager, {} "
                "warmup-disabled",
                get_segment_id(),
                cg_index,
                eager_field_count,
                disabled_field_count);
        }
    }
...
ChunkedSegmentSealedImpl::ApplyLoadDiff(milvus::OpContext* op_ctx,
    ...
    int64_t index_replace_ns = 0;
    int64_t reload_field_ns = 0;
    int64_t column_group_ns = 0;
    int64_t init_text_lob_ns = 0;
    int64_t binlog_load_ns = 0;
    int64_t binlog_replace_ns = 0;
    int64_t drop_index_ns = 0;
    int64_t text_index_ns = 0;
    int64_t json_stats_ns = 0;
    int64_t default_fill_ns = 0;
    int64_t create_text_index_ns = 0;
    int64_t drop_field_data_ns = 0;
    auto count_index_infos =
        [](const std::unordered_map<FieldId, std::vector<LoadIndexInfo>>&
               field_indexes) -> int64_t {
        int64_t total = 0;
        for (const auto& [_, indexes] : field_indexes) {
            total += static_cast<int64_t>(indexes.size());
        }
        return total;
    };
    auto timing_guard = folly::makeGuard([&]() {
        RecordApplyLoadDiffTiming(
            ...
            json_stats_ns,
            default_fill_ns,
            create_text_index_ns,
            drop_field_data_ns,
            count_index_infos(diff.indexes_to_load) +
                count_index_infos(diff.indexes_to_replace),
            static_cast<int64_t>(diff.column_groups_to_load.size() +
                                 diff.column_groups_to_replace.size() +
                                 diff.column_groups_to_lazyload.size() +
                                 diff.column_groups_to_lazyreplace.size()),
            static_cast<int64_t>(diff.binlogs_to_load.size() +
                                 diff.binlogs_to_replace.size()),
            static_cast<int64_t>(diff.text_indexes_to_load.size() +
                                 diff.text_indexes_to_create.size()),
            static_cast<int64_t>(diff.json_stats_to_load.size() +
                                 diff.json_stats_to_replace.size()));
    });
    ...
    index_load_ns = DurationSinceNs(stage_start);
    CheckCancellation(op_ctx, id_, "ChunkedSegmentSealedImpl::ApplyLoadDiff()");
    ...
    index_replace_ns = DurationSinceNs(stage_start);
    ...
    reload_field_ns = DurationSinceNs(stage_start);
    ...
    init_text_lob_ns = DurationSinceNs(stage_start);
    ...
    binlog_load_ns = DurationSinceNs(stage_start);
    CheckCancellation(op_ctx, id_, "ChunkedSegmentSealedImpl::ApplyLoadDiff()");
    ...
    binlog_replace_ns = DurationSinceNs(stage_start);
    ...
    drop_index_ns = DurationSinceNs(stage_start);
    ...
    text_index_ns = DurationSinceNs(stage_start);
    ...
    json_stats_ns = DurationSinceNs(stage_start);
    ...
    default_fill_ns = DurationSinceNs(stage_start);
    ...
    create_text_index_ns = DurationSinceNs(stage_start);
    ...
    drop_field_data_ns = DurationSinceNs(stage_start);
}
...
ChunkedSegmentSealedImpl::LoadColumnGroups(
    ...
    for (const auto& pair : cg_field_ids) {
        ...
        disabled_fields.reserve(field_ids.size());
        ...
        for (const auto& field_id : field_ids) {
            ...
            if (warmup_disabled) {
                disabled_fields.emplace_back(field_id);
            } else {
        ...
        if (!warmup_fields.empty()) {
            submit_load_group(cg_index,
                              ...
                              /*task_eager_load=*/true,
                              /*allow_match_field_lazy=*/false);
        }

        bool has_match_field = false;
        bool all_match_fields_allow_lazy = true;
        for (const auto& field_id : disabled_fields) {
            const auto& field_meta = (*schema_)[field_id];
            if (field_meta.enable_match()) {
                has_match_field = true;
                if (lazy_match_fields == nullptr ||
                    lazy_match_fields->find(field_id) ==
                        lazy_match_fields->end()) {
                    all_match_fields_allow_lazy = false;
                }
            }
        }
        if (!disabled_fields.empty()) {
            submit_load_group(cg_index,
                              std::move(disabled_fields),
                              /*task_eager_load=*/false,
                              has_match_field && all_match_fields_allow_lazy);
        }
    }
...
ChunkedSegmentSealedImpl::LoadColumnGroup(
    ...
    // The caller defines the loading unit. Lazy materialization preserves this
    // field set instead of splitting or merging it.
    ...
    auto warmup_disabled =
        effective_warmup_policy == CacheWarmupPolicy::CacheWarmupPolicy_Disable;
    auto should_defer =
        lazy_manifest_reader_enabled && warmup_disabled &&
        CanUseLazyManifestColumnGroup(field_metas, allow_match_field_lazy);

    if (lazy_manifest_reader_enabled && warmup_disabled && !should_defer) {
        for (const auto& [field_id, field_meta] : field_metas) {
            auto reason = LazyManifestFieldBlockReason(
                field_id, field_meta, allow_match_field_lazy);
            if (reason != nullptr) {
                LOG_DEBUG(
    ...
    // (for example, external single-field tasks), so non-eager entries use a
    // field suffix. Eager entries keep the existing unsuffixed key.
    std::string cache_key_suffix;
    if (!eager_load) {
    ...
    auto chunk_reader_factory = [this,
        ...
        return GetManifestChunkReader(
            column_groups, properties, reader_schema, index, needed_columns);
    };
    ManifestColumnGroupBuildContext context{
        get_segment_id(),
        index,
        needed_columns,
    ...
    ManifestColumnGroupBuildTiming build_timing;
    auto chunked_column_group =
        CreateManifestColumnGroup(context, &build_timing);
    reader_ns = build_timing.reader_ns;
    translator_ns = build_timing.translator_ns;
    cache_slot_ns = build_timing.cache_slot_ns;

    // Create ProxyChunkColumn for each field
    stage_start = std::chrono::steady_clock::now();
    for (const auto& field_id : milvus_field_ids) {
        const auto& field_meta = context.field_metas.at(field_id);
        auto column = std::make_shared<ProxyChunkColumn>(
            chunked_column_group, field_id, field_meta);
        auto data_type = field_meta.get_data_type();
...
ChunkedSegmentSealedImpl::Load(milvus::tracer::TraceContext& trace_ctx,
    ...
    int64_t start_log_ns = 0;
    int64_t prepare_info_ns = 0;
    int64_t diff_ns = 0;
    int64_t diff_log_ns = 0;
    int64_t apply_ns = 0;
    int64_t success_log_ns = 0;
    auto timing_guard = folly::makeGuard([&]() {
    ...
    auto stage_start = std::chrono::steady_clock::now();
    std::lock_guard<std::mutex> reopen_guard(reopen_mutex_);
    lock_wait_ns = DurationSinceNs(stage_start);
