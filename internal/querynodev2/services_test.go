...
func (suite *ServiceSuite) TestLoadIndex_Success() {
	...
	distResp := suite.getDataDistributionWithDeltaSupport(ctx, fullResp.GetLastModifyTs())
...
func (suite *ServiceSuite) TestGetDataDistribution_DeltaReportsStreamingDataCaughtUp() {
	...
	shardDelegator.UpdateTSafe(tsoutil.ComposeTSByTime(time.Now(), 0))
...
func (suite *ServiceSuite) TestGetDataDistribution_DeltaReconcilesReportedCatchingUpStatus() {
	ctx := context.Background()
	channel := "catching-up-snapshot-delta-channel"
	catchingUp := true
	...
	mockDelegator.EXPECT().CatchingUpStreamingData().RunAndReturn(func() bool {
		return catchingUp
	}).Maybe()
	...
	suite.Require().False(fullResp.GetIsDelta())
	suite.Require().NotZero(fullResp.GetLastModifyTs())
	suite.Require().Len(fullResp.GetLeaderViews(), 1)
	suite.True(fullResp.GetLeaderViews()[0].GetStatus().GetCatchingUpStreamingData())
