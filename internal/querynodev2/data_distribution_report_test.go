...
func TestDataDistributionDeltaTracker(t *testing.T) {
	...
	cache.markChannelViewUpsert("ch-1")
	if _, ok := cache.dirtyChannelViews["ch-1"]; !ok {
		t.Fatalf("expected channel upsert to dirty channel view")
	}
	if _, ok := cache.removedChannels["ch-1"]; ok {
		t.Fatalf("expected channel upsert to clear removed marker")
	}

	cache.markChannelViewRemove("ch-1")
	if _, ok := cache.dirtyChannelViews["ch-1"]; ok {
		t.Fatalf("expected channel remove to clear dirty channel view marker")
	}
	if _, ok := cache.removedChannels["ch-1"]; !ok {
	...
	cache.markChannelViewUpsert("ch-1")
	if _, ok := cache.dirtyChannelViews["ch-1"]; !ok {
		t.Fatalf("expected channel re-upsert to dirty channel view")
	}
	if _, ok := cache.removedChannels["ch-1"]; ok {
	...
	cache.markChannelViewRemove("ch-2")
	...
	if len(cache.dirtySegments) != 0 || len(cache.removedSegments) != 0 ||
		len(cache.dirtyChannelViews) != 0 || len(cache.removedChannels) != 0 {
		t.Fatalf("expected clearDelta to clear delta markers")
	}

	cache.reset()
	if cache.lastReportedTs != 10 {
		t.Fatalf("expected reset to keep reported timestamp")
	}
	if len(cache.dirtySegments) != 0 || len(cache.removedSegments) != 0 ||
		len(cache.dirtyChannelViews) != 0 || len(cache.removedChannels) != 0 {
		t.Fatalf("expected reset to clear delta markers")
	}

	var nilTracker *dataDistributionDeltaTracker
	nilTracker.markSegmentUpsert(1)
	nilTracker.markSegmentRemove(1)
	nilTracker.markChannelViewUpsert("ch-1")
	nilTracker.markChannelViewRemove("ch-1")

	emptyChannelTracker := newDataDistributionDeltaTracker()
	emptyChannelTracker.markChannelViewUpsert("")
	emptyChannelTracker.markChannelViewRemove("")
	if len(emptyChannelTracker.dirtyChannelViews) != 0 || len(emptyChannelTracker.removedChannels) != 0 {
...
func TestDataDistributionDeltaTrackerClearDeltaKeepsNewDelta(t *testing.T) {
	...
	reportChannelNames := make([]string, 0, len(cache.dirtyChannelViews))
	for channel := range cache.dirtyChannelViews {
	...
	if len(cache.dirtySegments) != 0 || len(cache.dirtyChannelViews) != 0 {
		t.Fatalf("expected clearDelta to swap out existing delta markers")
	}

	cache.markSegmentUpsert(2)
	cache.markChannelViewUpsert("ch-2")
	...
	if _, ok := cache.dirtyChannelViews["ch-2"]; !ok {
...
func TestDataDistributionDeltaTrackerFullReportInvalidatesDeltaBase(t *testing.T) {
	...
	if len(cache.dirtySegments) != 0 {
		t.Fatalf("expected full report reset to clear pending delta markers")
	}
}
...
func TestDataDistributionReportBuilders(t *testing.T) {
	...
	if value == nil || *value != 10 {
		t.Fatalf("expected pointer to int32 value")
	}
}
...
func TestMarkDataDistributionDelta(t *testing.T) {
	...
	if _, ok := node.distDeltaTracker.dirtyChannelViews["ch-1"]; !ok {
		t.Fatalf("expected full load to mark leader view")
	}
	if _, ok := node.distDeltaTracker.dirtyChannelViews["ch-2"]; ok {
		t.Fatalf("expected L0 segment to be skipped")
	}
	if _, ok := node.distDeltaTracker.dirtyChannelViews[""]; ok {
		t.Fatalf("expected empty channel to be skipped")
	}
	if _, ok := node.distDeltaTracker.dirtyChannelViews["ch-3"]; !ok {
		t.Fatalf("expected delta load to mark leader view")
	}
	if _, ok := node.distDeltaTracker.dirtyChannelViews["ch-4"]; ok {
		t.Fatalf("expected index load to be skipped")
	}
}
