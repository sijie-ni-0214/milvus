...
type dataDistributionDeltaTracker struct {
	...
	dirtyChannelViews map[string]struct{}
	removedChannels   map[string]struct{}

	reportedCatchingUpChannels map[string]struct{}
}
...
func newDataDistributionDeltaTracker() *dataDistributionDeltaTracker {
	return &dataDistributionDeltaTracker{
		...
		dirtyChannelViews:          make(map[string]struct{}),
		removedChannels:            make(map[string]struct{}),
		reportedCatchingUpChannels: make(map[string]struct{}),
	}
}

func (c *dataDistributionDeltaTracker) reset() {
	c.reportedCatchingUpChannels = make(map[string]struct{})
	c.clearDelta()
}
...
func (c *dataDistributionDeltaTracker) clearDelta() {
	...
	c.dirtyChannelViews = make(map[string]struct{})
...
func (c *dataDistributionDeltaTracker) markChannelViewUpsert(channel string) {
	if c == nil || channel == "" {
		return
	}
	c.mu.Lock()
	...
	c.dirtyChannelViews[channel] = struct{}{}
}

func (c *dataDistributionDeltaTracker) markChannelViewRemove(channel string) {
	if c == nil || channel == "" {
		return
	}
	c.mu.Lock()
	...
	delete(c.dirtyChannelViews, channel)
	c.removedChannels[channel] = struct{}{}
	delete(c.reportedCatchingUpChannels, channel)
}

func (c *dataDistributionDeltaTracker) recordReportedLeaderViewsLocked(leaderViews []*querypb.LeaderView) {
	if c.reportedCatchingUpChannels == nil {
		c.reportedCatchingUpChannels = make(map[string]struct{})
	}

	for _, leaderView := range leaderViews {
		channel := leaderView.GetChannel()
		if channel == "" {
			continue
		}
		if leaderView.GetStatus().GetCatchingUpStreamingData() {
			c.reportedCatchingUpChannels[channel] = struct{}{}
			continue
		}
		delete(c.reportedCatchingUpChannels, channel)
	}
