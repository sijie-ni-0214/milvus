...
func (loader *segmentLoader) requestResource(ctx context.Context, infos ...*querypb.SegmentLoadInfo) (requestResourceResult, error) {
	...
	if err := loader.checkLoadingResource(log, loadingUsage, maxSegmentSize, totalMemory, physicalMemoryUsage, physicalDiskUsage); err != nil {
		return result, err
	}
...
func (loader *segmentLoader) checkLoadingResource(
	...
	if memUsage == 0 || totalMem == 0 {
		return errors.New("get memory failed when checkLoadingResource")
	}
	...
	if paramtable.Get().QueryNodeCfg.TieredEvictionEnabled.GetAsBool() {
		...
		if ok := C.TryReserveLoadingResourceWithTimeout(C.CResourceUsage{
			...
			return fmt.Errorf("failed to reserve loading resource from caching layer, predictMemUsage = %v MB, predictDiskUsage = %v MB, memUsage = %v MB, diskUsage = %v MB, memoryThresholdFactor = %f, diskThresholdFactor = %f",
