...
func TestCompactSegmentLoadInfoForRuntime(t *testing.T) {
	...
	assert.Zero(t, compact.GetDbID())
	assert.Zero(t, compact.GetFlushTime())
	assert.Empty(t, compact.GetDeltalogs())
	assert.Empty(t, compact.GetCompactionFrom())
	assert.Nil(t, compact.GetDeltaPosition())
	assert.False(t, compact.GetUseTakeForOutput())
	assert.Zero(t, compact.GetEstimatedBytesPerRow())
