...
func gatherSegcoreMemoryMetrics() map[string]*dto.MetricFamily {
	...
	metrics := []struct {
		...
		{"milvus_segcore_deferred_business_index_count", "Live sealed business indexes whose translator and cache slot have not been created", uint64(cStats.deferred_business_index_count)},
		{"milvus_segcore_deferred_business_index_estimated_bytes", "Live deferred sealed business index descriptor bytes estimated from object size and retained load metadata", uint64(cStats.deferred_business_index_estimated_bytes)},
		{"milvus_segcore_deferred_text_index_count", "Live pre-built text indexes whose translator and cache slot have not been created", uint64(cStats.deferred_text_index_count)},
		{"milvus_segcore_deferred_text_index_estimated_bytes", "Live deferred text index descriptor bytes estimated from object size and retained load metadata", uint64(cStats.deferred_text_index_estimated_bytes)},
