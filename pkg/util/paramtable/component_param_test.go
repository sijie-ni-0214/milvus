...
func TestComponentParam_LazyManifestMetadataReadEnabled(t *testing.T) {
	Init()
	params := Get()
	item := &params.QueryNodeCfg.TieredLazyManifestMetadataReadEnabled
	t.Cleanup(func() { params.Reset(item.Key) })

	assert.Equal(t, "queryNode.segcore.tieredStorage.lazyManifestMetadataReadEnabled", item.Key)
	assert.Equal(t, "false", item.DefaultValue)
	assert.True(t, item.Export)
	assert.False(t, item.GetAsBool())
